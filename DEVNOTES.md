...
- **Deprecated-API check.** `.github/workflows/deprecated-api-check.yml` runs `lua-language-server` against `Ketho/vscode-wow-api`'s annotations and fails only on an actual `(deprecated)` diagnostic (everything else it reports is noise from incomplete annotation coverage, not real problems). Manual-only (`workflow_dispatch` from the Actions tab) - run it before tagging a release, alongside the usual in-game verification.
